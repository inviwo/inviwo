		...
		if (s_plugins) {
			...
			*/
			...
			if (!load_local_plugins_only) {
				...
				if (GetCurrentDirectory(2 * _MAX_PATH, LPWSTR(current_dir)) != 0) {
					if (GetModuleFileName(NULL, LPWCH(module), 2 * _MAX_PATH) != 0) {
						char *last_point = strrchr(module, '\\');

						if (last_point) {
							*last_point = '\0';

							bOk = SetCurrentDirectory(LPCWSTR(module));
						}
					}
				}
				...
				if (bOk) {
					SetCurrentDirectory(LPCWSTR(current_dir));
				}
			}
#endif // _WIN32
		}
