...
static void interleave(unsigned char* data, size_t bytes, unsigned int skip,
                       unsigned int block = 0) {
    deinterleave(data, bytes, skip, block, TRUE);
}

// write a Differential Data Stream
void writeDDSfile(const char* filename, unsigned char* data, size_t bytes, unsigned int skip,
                  unsigned int strip, int nofree) {
    int version = 1;

    unsigned char *ptr1, *ptr2;

    int pre1, pre2, act1, act2, tmp1, tmp2;
    ...
    while (cnt++ < bytes) {
        ...
        if (cnt1 + cnt2 < (1 << DDS_RL) &&
            (cnt1 + cnt2) * max(bits1, bits2) < cnt1 * bits1 + cnt2 * bits2 + DDS_RL + 3) {
            cnt2 += cnt1;
...
void writePNMimage(const char* filename, const unsigned char* image, unsigned int width,
                   unsigned int height, unsigned int components, int dds) {
    char str[DDS_MAXSTR];
    ...
    if (dds != 0)
        writeDDSfile(filename, data, strlen(str) + width * height * components, components, width);
    else
        writeRAWfile(filename, data, strlen(str) + width * height * components);
}

// read a possibly compressed PNM image
unsigned char* readPNMimage(const char* filename, unsigned int* width, unsigned int* height,
...
void writePVMvolume(const char* filename, const unsigned char* volume, unsigned int width,
                    unsigned int height, unsigned int depth, unsigned int components, float scalex,
                    float scaley, float scalez, const unsigned char* description,
                    const unsigned char* courtesy, const unsigned char* parameter,
    ...
    if (description == nullptr && courtesy == nullptr && parameter == nullptr && comment == nullptr)
        if (scalex == 1.0f && scaley == 1.0f && scalez == 1.0f)
            ...
            snprintf(str, DDS_MAXSTR, "PVM2\n%d %d %d\n%g %g %g\n%d\n", width, height, depth,
                     scalex, scaley, scalez, components);
    else
    ...
    if (description == nullptr && courtesy == nullptr && parameter == nullptr &&
        ...
        writeDDSfile(filename, data,
                     strlen(str) + width * height * depth * components + len1 + len2 + len3 + len4,
                     components, width);
    }
}

// read a compressed PVM volume
unsigned char* readPVMvolume(const char* filename, unsigned int* width, unsigned int* height,
                             unsigned int* depth, unsigned int* components, float* scalex,
...
inline int DDS_get(unsigned short int* volume, unsigned int width, unsigned int height,
                   unsigned int /*depth*/, unsigned int i, unsigned int j, unsigned int k) {
    return (volume[i + (j + k * height) * width]);
}

inline double DDS_getgrad(unsigned short int* volume, unsigned int width, unsigned int height,
                          unsigned int depth, unsigned int i, unsigned int j, unsigned int k) {
    double gx, gy, gz;

    if (i > 0)
        if (i < width - 1)
            gx = (DDS_get(volume, width, height, depth, i + 1, j, k) -
                  DDS_get(volume, width, height, depth, i - 1, j, k)) /
                 2.0;
        else
            gx = DDS_get(volume, width, height, depth, i, j, k) -
                 DDS_get(volume, width, height, depth, i - 1, j, k);
    else
        gx = DDS_get(volume, width, height, depth, i + 1, j, k) -
             DDS_get(volume, width, height, depth, i, j, k);

    if (j > 0)
        if (j < height - 1)
            gy = (DDS_get(volume, width, height, depth, i, j + 1, k) -
                  DDS_get(volume, width, height, depth, i, j - 1, k)) /
                 2.0;
        else
            gy = DDS_get(volume, width, height, depth, i, j, k) -
                 DDS_get(volume, width, height, depth, i, j - 1, k);
    else
        gy = DDS_get(volume, width, height, depth, i, j + 1, k) -
             DDS_get(volume, width, height, depth, i, j, k);
    ...
    if (k > 0)
        if (k < depth - 1)
            gz = (DDS_get(volume, width, height, depth, i, j, k + 1) -
                  DDS_get(volume, width, height, depth, i, j, k - 1)) /
                 2.0;
        else
            gz = DDS_get(volume, width, height, depth, i, j, k) -
                 DDS_get(volume, width, height, depth, i, j, k - 1);
    else
        gz = DDS_get(volume, width, height, depth, i, j, k + 1) -
             DDS_get(volume, width, height, depth, i, j, k);
...
unsigned char* quantize(unsigned char* volume, unsigned int width, unsigned int height,
                        unsigned int depth, BOOLINT nofree, BOOLINT linear, BOOLINT verbose) {
    unsigned int i, j, k;
    ...
    for (k = 0; k < depth; k++)
        for (j = 0; j < height; j++)
            for (i = 0; i < width; i++) {
                v = 256 * volume[2 * (i + (j + k * height) * width)] +
                    volume[2 * (i + (j + k * height) * width) + 1];
    ...
    if (verbose) printf("16 bit volume has scalar range=[%d,%d]\n", vmin, vmax);
    ...
    if (linear)
        ...
        for (k = 0; k < depth; k++)
            for (j = 0; j < height; j++)
                for (i = 0; i < width; i++)
                    err[DDS_get(volume3, width, height, depth, i, j, k)] +=
                        sqrt(DDS_getgrad(volume3, width, height, depth, i, j, k));
    ...
    for (k = 0; k < depth; k++)
        for (j = 0; j < height; j++)
            for (i = 0; i < width; i++)
                volume2[i + (j + k * height) * width] =
                    (int)(err[DDS_get(volume3, width, height, depth, i, j, k)] + 0.5);

    delete[] err;
    free(volume3);

    if (verbose) printf("quantized volume to 8 bit\n");
