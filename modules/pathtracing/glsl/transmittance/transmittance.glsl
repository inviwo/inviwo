    ...
    while (continueTraversal) {
        ...
        switch (METHOD) {
            ...
            case 2: {
                ...
                // Sample control opacity in the middle of the ray segment.
                // Clamp using upper bound as there seem to be some cases where
                // controlOpacity > upper bound
                float controlOpacity = min(
                    minMax.y,
                    getVoxel(avgOpacity, opacityParameters, raystart + raydir * (t0 + t1) * 0.5).x);
                //controlOpacity = (minMax.y + minMax.x)*0.5f;
                T *= residualRatioTrackingTransmittance(raystart, raydir, t0, t1, hashSeed, volume,
                                                        volumeParameters, transferFunction,
                                                        minMax.y, controlOpacity);
                }
                break;
            case 3:
            ...
            case 4: {
                ...
                float controlOpacity =
                    min(minMax.y, getVoxel(avgOpacity, opacityParameters, raystart + raydir * (t0 + t1) * 0.5).x);
                //controlOpacity = (minMax.y + minMax.x) * 0.5f;
                T *= poissonResidualTrackingTransmittance(
