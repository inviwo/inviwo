...
canvas = app.processorFactory.create("org.inviwo.CanvasGL", glm.ivec2(0, 200))
...
vertTextSpacing = 1.0 / (len(fontNames) + 0.5)

for i,(name, id) in enumerate(zip(fontNames, fontIdentifiers)) :
    p = app.processorFactory.create("org.inviwo.TextOverlayGL", glm.ivec2(0, processorSpacing * i))    
    p.identifier = name
    p.text.value = name
    ...
    p.getPropertyByIdentifier("position").value = glm.vec2(0, 1.0 - i * vertTextSpacing)
