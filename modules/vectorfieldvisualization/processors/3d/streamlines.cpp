...
namespace inviwo {
        ...
        , maxVelocity_("minMaxVelocity","Velocity Range","0",VALID)
    {
        ...
        seedPointsSpace_.addOption("texture", "Texture", StructuredCoordinateTransformer<3>::Space::Texture);
    ...
    void StreamLines::process() {
        ...
        for (const auto &seeds : seedPoints_) {
            for (auto &p : (*seeds)) {
                ...
                auto indexBuffer = mesh->addIndexBuffer(DrawType::LINES, ConnectivityType::STRIP_ADJACENCY);
                ...
                for (size_t i = 0; i < size; i++) {
                    ...
                    indexBuffer->add(static_cast<uint32_t>(mesh->addVertex(pos, glm::normalize(v), pos, c)));
