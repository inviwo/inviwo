...
#define TRACY_PLOT_CONFIG(x,y,z,w,a)
...
#define TRACY_SOURCE_CALLBACK_REGISTER(x,y)
#define TRACY_PARAMETER_REGISTER(x,y)
#define TRACY_PARAMETER_SETUP(x,y,z,w)
#define TRACY_IS_CONNECTED false
#define TRACY_SET_PROGRAM_NAME(x)
...
#  define TRACY_ZONE_NAMED( varname, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), TRACY_CALLSTACK, active )
#  define TRACY_ZONE_NAMED_N( varname, name, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), TRACY_CALLSTACK, active )
#  define TRACY_ZONE_NAMED_C( varname, color, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), TRACY_CALLSTACK, active )
#  define ZONE_NAMED_NC( varname, name, color, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), TRACY_CALLSTACK, active )

#  define TRACY_ZONE_TRANSIENT( varname, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), nullptr, 0, TRACY_CALLSTACK, active )
#  define TRACY_ZONE_TRANSIENT_N( varname, name, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), name, strlen( name ), TRACY_CALLSTACK, active )
#else
#  define TRACY_ZONE_NAMED( varname, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), active )
#  define TRACY_ZONE_NAMED_N( varname, name, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), active )
#  define TRACY_ZONE_NAMED_C( varname, color, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), active )
#  define ZONE_NAMED_NC( varname, name, color, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), active )

#  define TRACY_ZONE_TRANSIENT( varname, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), nullptr, 0, active )
#  define TRACY_ZONE_TRANSIENT_N( varname, name, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), name, strlen( name ), active )
#endif
...
#define TRACY_ZONE_SCOPED_NC( name, color ) ZONE_NAMED_NC( ___tracy_scoped_zone, name, color, true )
...
#define TRACY_LOCKABLE( type, varname ) tracy::Lockable<type> varname { [] () -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, #type " " #varname, TracyFile, TracyLine, 0 }; return &srcloc; }() }
#define TRACY_LOCKABLE_N( type, varname, desc ) tracy::Lockable<type> varname { [] () -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, desc, TracyFile, TracyLine, 0 }; return &srcloc; }() }
#define TRACY_SHARED_LOCKABLE( type, varname ) tracy::SharedLockable<type> varname { [] () -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, #type " " #varname, TracyFile, TracyLine, 0 }; return &srcloc; }() }
#define TRACY_SHARED_LOCKABLE_N( type, varname, desc ) tracy::SharedLockable<type> varname { [] () -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, desc, TracyFile, TracyLine, 0 }; return &srcloc; }() }
#define TRACY_LOCKABLE_BASE( type ) tracy::Lockable<type>
#define TRACY_SHARED_LOCKABLE_BASE( type ) tracy::SharedLockable<type>
#define TRACY_LOCK_MARK( varname ) static constexpr tracy::SourceLocationData __tracy_lock_location_##varname { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; varname.Mark( &__tracy_lock_location_##varname )
...
#define TRACY_PLOT_CONFIG( name, type, step, fill, color ) tracy::Profiler::ConfigurePlot( name, type, step, fill, color )
...
#  define TRACY_ZONE_NAMED_S( varname, depth, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), depth, active )
#  define TRACY_ZONE_NAMED_NS( varname, name, depth, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, 0 }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), depth, active )
#  define TRACY_ZONE_NAMED_CS( varname, color, depth, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { nullptr, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), depth, active )
#  define TRACY_ZONE_NAMED_NCS( varname, name, color, depth, active ) static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location,TracyLine) { name, TracyFunction,  TracyFile, (uint32_t)TracyLine, color }; tracy::ScopedZone varname( &TracyConcat(__tracy_source_location,TracyLine), depth, active )

#  define TRACY_ZONE_TRANSIENT_S( varname, depth, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), nullptr, 0, depth, active )
#  define TRACY_ZONE_TRANSIENT_NS( varname, name, depth, active ) tracy::ScopedZone varname( TracyLine, TracyFile, strlen( TracyFile ), TracyFunction, strlen( TracyFunction ), name, strlen( name ), depth, active )
...
#  define TRACY_SECURE_ALLOC_NS( ptr, size, depth, name ) tracy::Profiler::MemAllocCallstackNamed( ptr, size, depth, true, name )
#  define TRACY_SECURE_FREE_NS( ptr, depth, name ) tracy::Profiler::MemFreeCallstackNamed( ptr, depth, true, name )
...
#  define TRACY_ZONE_NAMED_NCS( varname, name, color, depth, active ) ZONE_NAMED_NC( varname, name, color, active )

#  define TRACY_ZONE_TRANSIENT_S( varname, depth, active ) TRACY_ZONE_TRANSIENT( varname, active )
#  define TRACY_ZONE_TRANSIENT_NS( varname, name, depth, active ) TRACY_ZONE_TRANSIENT_N( varname, name, active )

#  define TRACY_ZONE_SCOPED_S( depth ) TRACY_ZONE_SCOPED
#  define TRACY_ZONE_SCOPED_NS( name, depth ) TRACY_ZONE_SCOPED_N( name )
#  define TRACY_ZONE_SCOPED_CS( color, depth ) TRACY_ZONE_SCOPED_C( color )
...
#  define TRACY_ALLOC_NS( ptr, size, depth, name ) TRACY_ALLOC_N( ptr, size, name )
#  define TRACY_FREE_NS( ptr, depth, name ) TRACY_FREE_N( ptr, name )
#  define TRACY_SECURE_ALLOC_NS( ptr, size, depth, name ) TRACY_SECURE_ALLOC_N( ptr, size, name )
#  define TRACY_SECURE_FREE_NS( ptr, depth, name ) TRACY_SECURE_FREE_N( ptr, name )
...
#define TRACY_SOURCE_CALLBACK_REGISTER( cb, data ) tracy::Profiler::SourceCallbackRegister( cb, data )
#define TRACY_PARAMETER_REGISTER( cb, data ) tracy::Profiler::ParameterRegister( cb, data )
...
#define TRACY_SET_PROGRAM_NAME( name ) tracy::GetProfiler().SetProgramName( name );
