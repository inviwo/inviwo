...
try:
    ...
    def print_error(mess, **kwargs):
        print(colorama.Fore.RED + colorama.Style.BRIGHT
              + mess + colorama.Style.RESET_ALL, **kwargs)
                ...
                for (i, line) in enumerate(lines):
                    ...
                    if match:
                        if not match_in_file:
                            ...
                            match_in_file = True
                        matched = colorama.Fore.YELLOW + match.group(0) + colorama.Style.RESET_ALL
                        replaced = r.sub(colorama.Fore.RED + repl
                                         + colorama.Style.RESET_ALL, line.rstrip())
...
# n = find_files(["inviwo-dev", "inviwo-research"], source_extensions,
#                excludes=["*ext*", "*moc*", "*cmake*"])
# n = find_files(["inviwo-dev", "inviwo-research"], source_extensions,
#                excludes=["*/ext/*", "*moc_*", "*cmake*", "*/proteindocking/*",
#                          "*/proteindocking2/*", "*/genetree/*"])
# replace_matches(n, r"^(.*)ProcessorClassIdentifier\((\S+)\s*,\s*(.+)\);?\W*$",
#                 r"\1\2ProcessorClassVersion(\3, 0);\n")
