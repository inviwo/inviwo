...
def _CalculateReorderSpans(file_lines):
    ...
    while i < len(sorted_move_spans):
        ...
        for line_number in range(reorder_span_start, reorder_span_end):
            file_lines[line_number].reorder_span = (reorder_span_start, reorder_span_end)
        i += 1
        ...
        while end_line < len(file_lines):
            ...
            if line_info.deleted:
                ...
                if namespace_depth <= 0:
                    ...
                    start_line = _LineNumberStartingPrecedingComments(file_lines, start_line)
                    # And also blank lines.
                    while (start_line > 0 and file_lines[start_line-1].type == _BLANK_LINE_RE):
                        start_line -= 1
                    for line_number in range(start_line, end_line):
...
_C_SYSTEM_INCLUDE_KIND = 3        # e.g. #include <stdio.h>
_CXX_SYSTEM_INCLUDE_KIND = 4      # e.g. #include <vector>
_NONSYSTEM_INCLUDE_KIND = 5       # e.g. #include "bar.h"
_PROJECT_INCLUDE_KIND = 2         # e.g. #include "myproject/quux.h"
...
def _IsSystemInclude(line_info):
    headers = [
        ...
        "<cmath>", "<codecvt>", "<compare>", "<complex>", "<complex.h>", "<concepts>",
        "<condition_variable>", "<coroutine>", "<csetjmp>", "<csignal>", "<cstdarg>",
        "<cstdbool>", "<cstddef>", "<cstdint>", "<cstdio>", "<cstdlib>", "<cstring>",
        "<ctgmath>", "<ctime>", "<ctype.h>", "<cwchar>", "<cwctype>", "<cxxabi.h>",
        "<deque>", "<errno.h>", "<exception>", "<execution>", "<experimental>", "<ext>",
        "<fenv.h>", "<filesystem>", "<float.h>", "<format>", "<forward_list>", "<fstream>",
        "<functional>", "<future>", "<initializer_list>", "<inttypes.h>", "<iomanip>",
        "<ios>", "<iosfwd>", "<iostream>", "<istream>", "<iterator>", "<latch>", "<limits>",
        "<limits.h>", "<list>", "<locale>", "<locale.h>", "<map>", "<math.h>", "<memory>",
        "<module.modulemap>", "<mutex>", "<new>", "<numbers>", "<numeric>", "<optional>",
        "<ostream>", "<queue>", "<random>", "<ranges>", "<ratio>", "<regex>", "<scoped_allocator>",
        "<semaphore>", "<set>", "<setjmp.h>", "<shared_mutex>", "<span>", "<sstream>", "<stack>",
        "<stdbool.h>", "<stddef.h>", "<stdexcept>", "<stdint.h>", "<stdio.h>", "<stdlib.h>",
        "<streambuf>", "<string>", "<string.h>", "<string_view>", "<strstream>", "<system_error>",
        "<tgmath.h>", "<thread>", "<tuple>", "<type_traits>", "<typeindex>", "<typeinfo>",
    ...
    return line_info.type == _INCLUDE_RE and line_info.key in headers
...
def _IsSameProject(line_info, edited_file, project):
    ...
    return included_file.startswith("inviwo/") or included_file.startswith("modules/")
...
def FixFileLines(iwyu_record, file_lines, flags, fileinfo):
    ...
    for line in iwyu_record.includes_and_forward_declares_to_add:
        ...
        decorated_span = _DecoratedMoveSpanLines(iwyu_record, file_lines, [line_info], flags)
    ...
    decorated_move_spans.append(((len(file_lines), len(file_lines)), _EOF_KIND, '', []))
    if flags.reorder:
    ...
    while line_number < len(file_lines):
        ...
        new_lines = []
        while (decorated_move_spans and decorated_move_spans[0][0] == current_reorder_span):
            new_lines.extend(decorated_move_spans[0][3])   # the full content
            if (len(decorated_move_spans) > 1 and _ShouldInsertBlankLine(decorated_move_spans[0],
...
def FixOneFile(iwyu_record, file_contents, flags, fileinfo):
    ...
    old_lines = [fl.line for fl in file_lines if fl is not None and fl.line is not None]
