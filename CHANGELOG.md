...
A more comprehensive set of CMake presets has been added. There are now a set of building blocks:
`user` for cases where you only want to build and use Inviwo and `developer` in case where you also
 want to develop new modules and processors. 
 Further there are new generator presets for `msvc`, `xcode`, and `ninja` as well as "package" presets 
 for `vcpkg` and `apt`. 
 Finally, there are some defaults that combine these in convenient ways like
